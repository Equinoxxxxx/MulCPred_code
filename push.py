...
from receptive_field import compute_rf_loc_spatial
...
def update_prototypes_on_batch(search_batch_input,  # input of cur batch
    ...
    protoL_input_ = np.copy(protoL_input_torch.detach().cpu().numpy())  # B, C, T, H, W
    proto_dist_ = np.copy(proto_dist_torch.detach().cpu().numpy())  # B, np, T, H, W
    ...
    for j in range(n_prototypes):
        ...
        if batch_min_proto_dist_j < global_min_proto_dist[j]:
            batch_argmin_proto_dist_j = list(np.unravel_index(np.argmin(proto_dist_j, axis=None), proto_dist_j.shape))  # [b idx / n sample idx, (t idx), h idx, w idx] for feat map
            if class_specific:
                '''
                ...
                batch
                '''
                batch_argmin_proto_dist_j[0] = class_to_img_index_dict[target_class][batch_argmin_proto_dist_j[0]]  # n sample idx -> b idx
            ...
            global_min_fmap_patches[j] = batch_min_fmap_patch_j  # value to update the prototypes
            
            # get the receptive field boundary of the image patch
            # that generates the representation
            protoL_rf_info = prototype_network_parallel.module.proto_layer_rf_info  # [n, j, r, start]  **************************
            rf_prototype_j = compute_rf_loc_spatial(search_batch.size(2), batch_argmin_proto_dist_j, protoL_rf_info)  # [b idx, t, b, l, r] ********************
            ...
            proto_dist_img_j = proto_dist_[img_index_in_batch, j, :, :]  # dist map of the particular closest sample
