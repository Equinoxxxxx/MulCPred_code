...
def get_args():
    ...
    parser.add_argument('--contrast_mode', type=str, default='pair_wise')
...
def main(rank, world_size, args):
    ...
    if apply_tte:
        tte = [0, (obs_len+pred_len)*(obs_interval+1)*3+1]
    if test_apply_tte:
        test_tte = [0, (obs_len+pred_len)*(obs_interval+1)*3+1]
    ...
    if ddp:
        for d in pre_datasets:
            ...
            pre_loaders.append(torch.utils.data.DataLoader(d, 
        ...
        for d in train_datasets:
            sampler = DistributedSampler(d)
            train_loaders.append(torch.utils.data.DataLoader(d, 
                                                             batch_size=batch_size, 
                                                             shuffle=False, 
                                                             num_workers=dataloader_workers, 
                                                             pin_memory=True, 
        ...
        for d in val_datasets:
            sampler = DistributedSampler(d)
            val_loaders.append(torch.utils.data.DataLoader(d, 
                                                             batch_size=batch_size, 
                                                             shuffle=False, 
                                                             num_workers=dataloader_workers, 
                                                             pin_memory=True, 
        ...
        for d in test_datasets:
            sampler = DistributedSampler(d)
            test_loaders.append(torch.utils.data.DataLoader(d, 
                                                            batch_size=batch_size, 
                                                            shuffle=False, 
                                                            num_workers=dataloader_workers, 
                                                            pin_memory=True, 
    ...
    if scheduler == 'step':
        lr_scheduler = torch.optim.lr_scheduler.StepLR(optimizer=optimizer, step_size=lr_step_size, gamma=lr_step_gamma)
