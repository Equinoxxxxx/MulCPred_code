...
from _proto_model import MotionPPNet, SkeletonPNet
from _backbones import create_backbone, record_conv2d_info, record_sp_conv3d_info_w, record_t_conv3d_info, BackboneOnly
...
def main():
    ...
    parser.add_argument('--backbone_name', type=str, default='SK')
    ...
    log('----------------------------Load data-----------------------------')
    # train set
    train_dataset = PIEDataset(obs_len=obs_len, do_balance=balance_train, subset='train', bbox_size=ped_img_size, data_split_type=data_split_type,
                                img_norm_mode=normalize_img_mode, resize_mode=resize_mode, max_occ=max_occ, use_img=False, use_skeleton=True, min_wh=min_wh)
    ...
    train_push_loader = torch.utils.data.DataLoader(train_dataset, batch_size=project_batch_size, shuffle=shuffle,
                                                num_workers=4, pin_memory=False)
    # test set
    test_dataset = PIEDataset(obs_len=obs_len, do_balance=balance_test, subset='test', bbox_size=ped_img_size,  data_split_type=data_split_type,
                                img_norm_mode=normalize_img_mode, resize_mode=resize_mode, max_occ=max_occ, use_img=False, use_skeleton=True, min_wh=min_wh)
    ...
    if is_prototype_model:
        ...
        sp_proto_layer_rf_info = compute_proto_layer_rf_info_v2(input_size=obs_len,
        ...
        ppnet = SkeletonPNet(backbone=backbone, 
                            skeleton_seq_shape=(2, obs_len, 17), 
                            p_per_cls=p_per_cls, 
    ...
    if is_prototype_model:
        joint_optimizer_specs = [{'params': ppnet.backbone.parameters(), 'lr': joint_lr, 'weight_decay': 1e-3}, # bias are now also being regularized
                                {'params': ppnet.add_on_layers.parameters(), 'lr': joint_optimizer_lrs['add_on_layers'], 'weight_decay': 1e-3},
                                {'params': ppnet.prototype_vectors, 'lr': joint_optimizer_lrs['prototype_vectors']},
                                {'params': ppnet.last_layer.parameters(), 'lr': last_lr}
                                ]
        joint_optimizer = torch.optim.Adam(joint_optimizer_specs)
        joint_lr_scheduler = torch.optim.lr_scheduler.StepLR(joint_optimizer, step_size=lr_step_size, gamma=0.1)
    ...
    for epoch in range(1, epochs+1):
        ...
        if is_prototype_model:
            if epoch <= warm_epochs:
                ...
                train_res = tnt.train(model=ppnet_multi, dataloader=train_loader, optimizer=warm_optimizer,
                            class_specific=class_specific, coefs=coefs, log=log, is_prototype_model=is_prototype_model, v_data_type='obs_skeletons')
            else:
                # 训练整个模型
                ...
                train_res = tnt.train(model=ppnet_multi, dataloader=train_loader, optimizer=joint_optimizer,
                            class_specific=class_specific, coefs=coefs, log=log, is_prototype_model=is_prototype_model, v_data_type='obs_skeletons')
                print('grad to input', torch.mean(ppnet_multi.module.backbone.conv1.weight.grad))
                print('grad to backbone', torch.mean(ppnet_multi.module.backbone.conv3.weight.grad))
                print('grad to add on', torch.mean(list(ppnet_multi.module.add_on_layers.parameters())[-1].grad))
        ...
        if epoch%test_every == 0:
            ...
            test_res = tnt.test(model=ppnet_multi, dataloader=test_loader,
                            class_specific=class_specific, log=log, is_prototype_model=is_prototype_model, v_data_type='obs_skeletons')
        ...
        if epoch >= push_start and (epoch)%push_start == 0 and is_prototype_model:
            ...
            _project_prototypes.push_sk_protos(
            ...
            res_test = tnt.test(model=ppnet_multi, dataloader=test_loader,
                            class_specific=class_specific, log=log, v_data_type='obs_skeletons')
            acc_test = res_test[0]
            ...
            if prototype_activation_function != 'linear':
                ...
                for i in range(1, linear_epochs+1):
                    ...
                    res_train = tnt.train(model=ppnet_multi, dataloader=train_loader, optimizer=last_layer_optimizer,
                                class_specific=class_specific, coefs=coefs, log=log, v_data_type='obs_skeletons')
                    print('grad to input', torch.mean(ppnet_multi.module.backbone.conv1.weight.grad))
                    print('grad to backbone', torch.mean(ppnet_multi.module.backbone.conv3.weight.grad))
                    print('grad to add on', torch.mean(list(ppnet_multi.module.add_on_layers.parameters())[-1].grad))
                    if i%test_every == 0:
