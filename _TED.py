...
class DecoderLayer(nn.Module):
  ...
  def forward(self, x, enc_output, combined_mask, padding_mask):
    ...
    attn1, _ = self.mha1(x, x, x, combined_mask)  # (batch_size, target_seq_len, d_model)
    attn1 = self.dropout1(attn1)
    out1 = self.layernorm1(attn1 + x)
    
    attn2, _ = self.mha2(out1, enc_output, enc_output, padding_mask)  # (batch_size, target_seq_len, d_model)
    attn2 = self.dropout2(attn2)
    out2 = self.layernorm2(attn2 + out1)  # (batch_size, target_seq_len, d_model)
    
    ffn_output = self.cnn(out2)  # (batch_size, target_seq_len, d_model)
...
class Encoder(nn.Module):
    def __init__(self, num_layers, d_model, d_input, num_heads, dff, maximum_position_encoding, rate=0.1):
        ...
        self.decoding = nn.Linear(self.d_model, 4)
    ...
    def forward(self, x, mask):
        ...
        action = self.decoding(x)
        action = torch.mean(action, dim=1)
        action = self.dense(action)
        action = self.activation(action)

        return x, action, attention_weights  # (batch_size, input_seq_len, d_model)
...
class Transformer(nn.Module):
  def __init__(self, num_layers, d_model, d_input, num_heads, dff, pe_input, pe_target, rate=0.1):
    super().__init__()
    ...
    self.encoder = Encoder(num_layers, d_model, d_input, num_heads, dff, pe_input, rate)

    self.decoder = Decoder(num_layers, d_model, d_input, num_heads, dff, pe_target, rate)

    self.final_layer = nn.Linear(d_model, d_input)
    
  def forward(self, inp_enc, inp_dec, combined_mask, dec_padding_mask = None, enc_padding_mask = None):
      
    enc_output, action, attention_weights = self.encoder(inp_enc, enc_padding_mask)  # (batch_size, inp_seq_len, d_model)
    
    # dec_output.shape == (batch_size, tar_seq_len, d_model)
    dec_output = self.decoder(inp_dec, enc_output, combined_mask, dec_padding_mask)
    
    final_output = self.final_layer(dec_output)  # (batch_size, tar_seq_len, target_vocab_size)

    return final_output, action