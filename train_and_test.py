...
def _train_or_test(model, dataloader, optimizer=None, class_specific=True, use_l1_mask=True,
    ...
    for i, (image, label) in enumerate(dataloader):
        input = image.cuda()
        target = label.cuda()  # real numbers, not one hot
        ...
        with grad_req:
            ...
            if class_specific:
                max_dist = (model.module.prototype_shape[1]
                            ...
                            * model.module.prototype_shape[3])  # C * 1 * 1 = C  (results of add_on_layer are sigmoid-ed) ****************************

                # prototypes_of_correct_class is a tensor of shape batch_size * num_prototypes
                # calculate cluster cost
                prototypes_of_correct_class = torch.t(model.module.prototype_class_identity[:,label]).cuda()  # (np, B) ->transpose-> (B, np)
