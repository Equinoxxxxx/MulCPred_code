...
class PPNet(nn.Module):
    ...
    def __init__(self, features, img_size, prototype_shape,
        ...
        self.num_prototypes = prototype_shape[0]  # 10 per cls by default, 2000 for total
        ...
        if features_name.startswith('VGG') or features_name.startswith('RES'):
            first_add_on_layer_in_channels = \
                [i for i in features.modules() if isinstance(i, nn.Conv2d)][-1].out_channels  # channel of the last conv
    ...
    def _l2_convolution(self, x):
        ...
        '''
        x2 = x ** 2  # B * C * H * W
        x2_patch_sum = F.conv2d(input=x2, weight=self.ones)  # B * 2000 * H * W

        p2 = self.prototype_vectors ** 2  # 2000 * C * 1 * 1
    ...
    def forward(self, x):
        distances = self.prototype_distances(x) # B, np, H, W
        '''
